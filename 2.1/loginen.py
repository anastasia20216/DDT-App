...
# Test the function
    ...
    if verify_password(password, password_to_check):
        print("Password is correct.")
    else:
        print("Password is incorrect.")
    
    if result:
        c.execute("UPDATE users SET active_user = 'yes' WHERE username=?", (username))
        conn.commit()
        conn.close()
...
label.grid(row=1, column=0, columnspan=2, padx=5, pady=50, sticky='nesw')
# Create and place the username label and entry
username_label = Label(root, text="Username:", background='#FAF9F6')
username_label.grid(row=2, column=0, padx=10, pady=10, sticky='e')
...
password_entry.grid(row=3, column=1, padx=5, pady=15, sticky='w')
...
registration_button.grid(row=5, column=0, columnspan=2, pady=5)
